<?php include '../../base3a.php' ?>

<?php startblock('page-title') ?>
  Welcome Daniel Wang!
<?php endblock() ?>
<?php emptyblock('search') ?>
<?php startblock('sub-page') ?>
<ul class="nav nav-pills">
  <li class="active"><a href="#"> <i class="pu-icon pu-icon-home"></i> Home</a></li>
  <li><a href="#"><i class="pu-icon pu-icon-profile"></i>  Profile</a></li>
  <li><a href="#"><i class="pu-icon pu-icon-resume"></i> Resume</a></li>
  <li><a href="#"><i class="pu-icon pu-icon-person"></i>  Account</a></li>
</ul>
<?php endblock() ?>
<?php startblock('page-body') ?>
<div class="alert alert-success">
  <strong>Congratulations!</strong>
  Totam officiis dolorum voluptatibus maxime molestiae iste. Lorem ipsum dolor sit amet, consectetuer adipiscin
  <button href="#" class="btn btn-sm btn-success">
    Onboarding Portal
  </button>
</div>
<hr>
<h3>Incomplete Applications</h3>
<hr>
<div class="row">
  <div class="col-sm-4">
    <article class="pu-card">
        <header class="pu-card-header">
            <h4>
                <a href="application">Business Development Executive (512287)</a>
            </h4>
            <p>
                <i class="fa fa-map-marker" aria-hitden="true"></i>
                    Melbourne, Australia
            </p>
            <p class="gray">
                <small>
                    <i class="fa fa-clock-o" aria-hitden="true"></i>
                    Submitted 29 March 2017 at 9:24pm AEST</small>
            </p>
            <p>
                <button class="btn btn-xs btn-outline btn-primary" type="button">Update</button>
                <button class="btn btn-xs btn-outline btn-danger" type="button">Withdraw</button>
            </p>
        </header>
    </article>
  </div>
</div>
<h3>Submitted Applications</h3>
<hr>
<div class="row">
  <div class="col-sm-4">
    <article class="pu-card">
        <header class="pu-card-header">
            <h4>
                <a href="application">Business Development Executive (512287)</a>
            </h4>
            <p>
                <i class="fa fa-map-marker" aria-hitden="true"></i>
                    Melbourne, Australia
            </p>
            <p class="gray">
                <small>
                    <i class="fa fa-clock-o" aria-hitden="true"></i>
                    Submitted 29 March 2017 at 9:24pm AEST</small>
            </p>
            <p>
                <button class="btn btn-xs btn-outline btn-primary" type="button">Update</button>
                <button class="btn btn-xs btn-outline btn-danger" type="button">Withdraw</button>
            </p>
        </header>
    </article>
  </div>
  <div class="col-sm-4">
    <article class="pu-card">
        <header class="pu-card-header">
            <h4>
                <a href="application">Sales Manager (512287)</a>
            </h4>
            <p>
                <i class="fa fa-map-marker" aria-hitden="true"></i>
                    Melbourne, Australia
            </p>
            <p class="gray">
                <small>
                    <i class="fa fa-clock-o" aria-hitden="true"></i>
                    Submitted 29 March 2017 at 9:24pm AEST</small>
            </p>
            <p>
                <button class="btn btn-xs btn-outline btn-primary" type="button">Update</button>
                <button class="btn btn-xs btn-outline btn-danger" type="button">Withdraw</button>
            </p>
        </header>
    </article>
  </div>
  <div class="col-sm-4">
    <article class="pu-card">
        <header class="pu-card-header">
            <h4>
                <a href="application">Sales Representitive (512287) - NSW</a>
            </h4>
            <p>
                <i class="fa fa-map-marker" aria-hitden="true"></i>
                    Sydney, Australia
            </p>
            <p class="gray">
                <small>
                    <i class="fa fa-clock-o" aria-hitden="true"></i>
                    Submitted 29 March 2017 at 9:24pm AEST</small>
            </p>
            <p>
                <button class="btn btn-xs btn-outline btn-primary" type="button">Update</button>
                <button class="btn btn-xs btn-outline btn-danger" type="button">Withdraw</button>
            </p>
        </header>
    </article>
  </div>
</div>
<?php endblock() ?>
