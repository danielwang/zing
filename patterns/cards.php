<?php include '../base.php';?>
<?php startblock('page-body');?>
<h1>Cards</h1>
<div class="row">
    <h3 class="col-sm-12">Small</h3>
    <div class="col-sm-3">
        <article class="pu-card">
            <header class="pu-card-header">
                <main class="pu-card-header-main">
                    <h4>
                        <a href="#_">John Smith</a>
                        <span class="label label-info">new</span>
                    </h4>
                    <h5>
                        Sales Manager
                    </h5>
                    <p class="gray">
                        <small>
                            <i class="fa fa-clock-o" aria-hitden="true"></i>
                            Last updated 08 March 2017</small>
                    </p>
                </main>
            </header>
        </article>
    </div>
    <div class="col-sm-3 col-sm-offset-1">
        <article class="pu-card">
            <header class="pu-card-header">
                <div class="actions">
                    <div class="checkbox checkbox-circle checkbox-success">
                        <input type="checkbox">
                        <label></label>
                    </div>
                    <div class="more">
                        <a href="#" data-toggle="dropdown" class="dropdown-toggle">
                            <i class="fa fa-ellipsis-h fa-lg" aria-hitden="true"></i>
                        </a>
                        <ul class="dropdown-menu dropdown-menu-right">
                            <li>
                                <a href="#">Action 1</a>
                            </li>
                            <li>
                                <a href="#">Action 2</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <aside class="pu-card-header-aside"><img class="pu-avatar pu-avatar" src="../img/emp/emp3.png"></aside>
                <main class="pu-card-header-main">
                    <h4>
                        <a href="#_">Deborah Mason</a>
                    </h4>
                    <h5>SVP Global Talent</h5>
                    <p class="gray">
                        <small>
                            <i class="fa fa-map-marker" aria-hitden="true"></i>
                            Melbourne, Australia</small>
                    </p>
                    <p>
                        <button class="btn btn-xs btn-outline btn-primary" type="button">Quick action</button>
                    </p>
                </main>
            </header>
        </article>
    </div>
    <div class="col-sm-3 col-sm-offset-1">
        <div class="pu-cards">
            <article class="pu-card">
                <header class="pu-card-header">
                    <aside class="pu-card-header-aside"><img class="pu-avatar" src="../img/emp/emp2.png">
                    </aside>
                    <main class="pu-card-header-main">
                        <h4>
                            <a href="#_">Marcus McNamara</a>
                        </h4>
                        <h5>Presales Engineer</h5>
                        <p class="gray">
                            <small>
                                <i class="fa fa-map-marker" aria-hitden="true"></i>
                                Melbourne, Australia</small>
                        </p>
                    </main>
                </header>
            </article>
            <article class="pu-card">
                <header class="pu-card-header">
                    <aside class="pu-card-header-aside"><img class="pu-avatar" src="../img/emp/emp33.png">
                    </aside>
                    <main class="pu-card-header-main">
                        <h4>
                            <a href="#_">Daniel Wang</a>
                        </h4>
                        <h5>UX Specialist</h5>
                        <p class="gray">
                            <small>
                                <i class="fa fa-map-marker" aria-hitden="true"></i>
                                Melbourne, Australia</small>
                        </p>
                    </main>
                </header>
            </article>
            <article class="pu-card">
                <header class="pu-card-header">
                    <aside class="pu-card-header-aside"><img class="pu-avatar" src="../img/emp/emp13.png">
                    </aside>
                    <main class="pu-card-header-main">
                        <h4>
                            <a href="#_">Matt Birman</a>
                        </h4>
                        <h5>Software Engineer</h5>
                        <p class="gray">
                            <small>
                                <i class="fa fa-map-marker" aria-hitden="true"></i>
                                Melbourne, Australia</small>
                        </p>
                    </main>
                </header>
            </article>
        </div>
    </div>
</div>

<div class="row">
    <h3 class="col-sm-12">Large</h3>
    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <h4>
                    <a href="#_">Jenifer Smith</a>
                </h4>
                <h5>
                    <a href="#">Sales Manager</a>
                    <small class="gray">
                        @ Francotyp-Postalia Ag &amp; Co. Kg</small>
                </h5>
                <p>Flags:
                    <i class="fa fa-plane yellow" aria-hitden="true"></i>
                    <i class="fa fa-flag green"></i>
                    <i class="fa fa-phone red"></i>
                    <i class="fa fa-trophy purple"></i>
                </p>
            </header>
            <section class="pu-card-content">
                <h6>Past roles</h6>
                <p>
                    <b>Operations Director
                    </b>
                    <span>
                        @ Francotyp-Postalia Ag &amp; Co. Kg.</span>
                    <small class="gray">Jan 2003 to Sept 2004 (1 year 8 months)</small>
                </p>
                <p>
                    <b>Operations Director
                    </b>
                    <span>
                        @ At&amp;T Labs Research, Florham</span><br>
                    <small class="gray">June 1997 to July 1999 (2 years 1 month)</small>
                </p>
                <p>
                    <a href="#">More</a>
                </p>
            </section>
        </article>
    </div>
    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <aside class="pu-card-header-aside"><img class="pu-avatar" src="../img/emp/emp6.png"></aside>
                <main class="pu-card-header-main">
                    <h4>
                        <a href="#_">Tal Robart</a>
                    </h4>
                    <h5>Chef Technology Officer</h5>
                    <p>
                        <span class="label label-info">agility
                        </span>
                        <span class="label label-danger">mobile
                        </span>
                        <span class="label label-purple">client facing
                        </span>
                    </p>
                </main>
            </header>
            <section class="pu-card-content">
                <p>
                    <i class="fa fa-fw fa-envelope"></i>
                    Tal.Robart@pageuppeople.com
                </p>
                <p>
                    <i class="fa fa-fw fa-phone"></i>
                    +61 0400 000 000
                </p>
                <p>
                    <i class="fa fa-fw fa-map-marker"></i>
                    91 William Street, Melbourne
                </p>
                <p>
                    <i class="fa fa-fw fa-suitcase"></i>
                    Technology</p>
                <p>
                    <i class="fa fa-fw fa-user"></i>
                    Manager: John Smith</p>
            </section>
            <footer class="pu-card-footer">
                <button class="btn btn-sm btn-primary">Action</button>
            </footer>
        </article>
    </div>
    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <div class="actions">
                    <div class="checkbox checkbox-circle checkbox-success">
                        <input type="checkbox">
                        <label></label>
                    </div>
                    <div class="more">
                        <a href="#" data-toggle="dropdown" class="dropdown-toggle">
                            <i class="fa fa-ellipsis-h fa-lg" aria-hitden="true"></i>
                        </a>
                        <ul class="dropdown-menu dropdown-user">
                            <li>
                                <a href="#">Action 1</a>
                            </li>
                            <li>
                                <a href="#">Action 2</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <aside class="pu-card-header-aside"><img class="pu-avatar pu-avatar" src="../img/emp/emp7.png"></aside>
                <main class="pu-card-header-main">
                    <h4>
                        <a href="#_">Theo Willetts</a>
                    </h4>
                    <h5>Global Support Team Lead</h5>
                    <p>
                        <span class="label label-yellow">leadership
                        </span>
                        <span class="label label-success">management
                        </span>
                        <span class="label label-purple">Analysis
                        </span>
                    </p>
                </main>
            </header>
            <section class="pu-card-content">
                <ul class="nav nav-tabs nav-justified nav-line">
                    <li class="active">
                        <a href="#1a">Details</a>
                    </li>
                    <li>
                        <a href="#2a">Activity</a>
                    </li>
                    <li>
                        <a href="#3a">Documents
                            <sup class="badge">2</sup>
                        </a>
                    </li>
                </ul>
                <div class="tab-content clearfix">
                    <div class="tab-pane active" id="1a">
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
                    </div>
                    <div class="tab-pane" id="2a">
                        <p>Inquit, dasne adolescenti veniam? Mihi quidem Homerus huius modi quitdam Lorem ipsum dolor sit amet, consectetur adipiscing elit. De illis, cum volemus. Inquit, dasne adolescenti
                        </p>
                    </div>
                    <div class="tab-pane" id="3a">
                        <p>Mihi quidem Homerus huius modi quitdam Lorem ipsum dolor sit amet, consectetur adipiscing elit. De illis, cum volemus. Inquit, dasne adolescenti
                        </p>
                    </div>
                </div>
            </section>
            <footer class="pu-card-footer">
                <button class="btn btn-sm btn-primary">Action</button>
            </footer>
        </article>
    </div>
</div>
<h2>Job Cards</h2>
<div class="row">
    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <h4>
                    <a href="#_">Business Development Executive</a>
                </h4>
                <p>
                    <span class="label label-info">Interviewing</span>
                </p>
                <p class="gray">
                    <small>
                        Job No.: 512829</small>
                    <small>
                        Position No.: 5</small>
                </p>
            </header>
        </article>
    </div>

    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <div class="actions">
                    <div class="checkbox checkbox-circle checkbox-success">
                        <input type="checkbox">
                        <label></label>
                    </div>
                    <div class="more">
                        <a href="#" data-toggle="dropdown" class="dropdown-toggle">
                            <i class="fa fa-ellipsis-h fa-lg" aria-hitden="true"></i>
                        </a>
                        <ul class="dropdown-menu dropdown-menu-right">
                            <li>
                                <a href="#">Close job</a>
                            </li>
                            <li>
                                <a href="#">Copy job</a>
                            </li>
                            <li>
                                <a href="#">Invite</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <aside class="pu-card-header-aside">
                    <a class="gray" href="#">
                        <i class="pu-icon pu-icon-team pu-icon-2x"></i>
                        <span class="app">36</span>
                    </a>
                </aside>
                <main class="pu-card-header-main">
                    <h4>
                        <a href="#_">Sales Manager</a>
                    </h4>
                    <p>
                        <span class="label label-info">Approved to advertise</span>
                    </p>
                    <p class="gray">
                        <small>
                            Job No.: 512829</small>
                        <small>
                            Position No.: 5</small>
                    </p>
                </main>
            </header>
            <section class="pu-card-content">
                <ul class="nav nav-tabs nav-justified nav-line">
                    <li class="active">
                        <a href="#1f">Details</a>
                    </li>
                    <li>
                        <a href="#2f">Cost</a>
                    </li>
                    <li>
                        <a href="#3f">Notes
                            <sup class="badge">2</sup>
                        </a>
                    </li>
                </ul>
                <div class="tab-content clearfix">
                    <div class="tab-pane active" id="1f">
                        <table>
                            <tr>
                                <td>Location:
                                </td>
                                <td>Melbourne</td>
                            </tr>
                            <tr>
                                <td>Business Unit:
                                </td>
                                <td>Sale Team</td>
                            </tr>
                            <tr>
                                <td>Recruiter:
                                </td>
                                <td>
                                    <a href="#">Bob Smith</a>
                                </td>
                            </tr>
                            <tr>
                                <td>Hiring Manager:
                                </td>

                                <td>
                                    <a href="#">Jenifer Smith</a>
                                </td>
                            </tr>
                        </table>
                    </div>
                    <div class="tab-pane" id="2f">
                        <p>Inquit, dasne adolescenti veniam? Mihi quidem Homerus huius modi quitdam Lorem ipsum dolor sit amet, consectetur adipiscing elit. De illis, cum volemus. Inquit, dasne adolescenti
                        </p>
                    </div>
                    <div class="tab-pane" id="3f">
                        <p>Mihi quidem Homerus huius modi quitdam Lorem ipsum dolor sit amet, consectetur adipiscing elit. De illis, cum volemus. Inquit, dasne adolescenti
                        </p>
                    </div>
                </div>
            </section>
        </article>
    </div>
    <div class="col-sm-4">
        <div class="pu-cards">
            <article class="pu-card">
                <header class="pu-card-header">
                    <aside class="pu-card-header-aside">
                        <a class="gray" href="#">
                            <i class="pu-icon pu-icon-team pu-icon-2x"></i>
                            <span class="app">36</span>
                        </a>
                    </aside>
                    <main class="pu-card-header-main">
                        <h4>
                            <a href="#_">Senior HR Consultant</a>
                        </h4>
                        <p>
                            <span class="label label-info">Sourcing</span>
                        </p>
                        <p class="gray">
                            <small>
                                Job No.: 512829</small>
                            <small>
                                Position No.: 5</small>
                        </p>
                    </main>
                </header>
            </article>
            <article class="pu-card">
                <header class="pu-card-header">
                    <aside class="pu-card-header-aside">
                        <a class="gray" href="#">
                            <i class="pu-icon pu-icon-team pu-icon-2x"></i>
                            <span class="app">22</span>
                        </a>
                    </aside>
                    <main class="pu-card-header-main">
                        <h4>
                            <a href="#_">Project Manager</a>
                        </h4>
                        <p>
                            <span class="label label-info">Interviewing</span>
                        </p>
                        <p class="gray">
                            <small>
                                Job No.: 512829</small>
                            <small>
                                Position No.: 5</small>
                        </p>
                    </main>
                </header>
            </article>
            <article class="pu-card">
                <header class="pu-card-header">
                    <aside class="pu-card-header-aside">
                        <a class="gray" href="#">
                            <i class="pu-icon pu-icon-team pu-icon-2x"></i>
                            <span class="app">15</span>
                        </a>
                    </aside>
                    <main class="pu-card-header-main">
                        <h4>
                            <a href="#_">Software Developer</a>
                        </h4>
                        <p>
                            <span class="label label-info">Offer made</span>
                        </p>
                        <p class="gray">
                            <small>
                                Job No.: 512829</small>
                            <small>
                                Position No.: 5</small>
                        </p>
                    </main>
                </header>
            </article>
        </div>
    </div>
</div>
<h2>Journal Card
</h2>
<div class="row">
    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <h4>
                    <a href="#_">Sales presentation to Aqua Corp</a>
                </h4>
                <p class="gray">
                    <small>
                        <i class="fa fa-clock-o"></i>
                        19 Oct 15, 03:53 PM</small>
                </p>
            </header>
            <section class="pu-card-content">
                Delivered a sales presentation. Got great feedback about my delivery from Senior Management. #dayne gave feedback about my presentation skills. Commented I need to work on time management and that I should ...
            </section>
        </article>
    </div>

    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <h4>
                    <a href="#_">Sales presentation to Aqua Corp</a>
                </h4>
                <p class="gray">
                    <small>
                        <i class="fa fa-clock-o"></i>
                        19 Oct 15, 03:53 PM</small>
                </p>
            </header>
            <figure class="pu-card-figure">
                <img class="featured" src="../img/image.jpg" alt="">
            </figure>
            <section class="pu-card-content">
                Delivered a sales presentation. Got great feedback about my delivery from Senior Management. #dayne gave feedback about my presentation skills. Commented I need to work on time management and that I should ...
            </section>
            <footer class="pu-card-footer">
                <p>
                    <i class="fa fa-tags"></i>
                    <span class="pu-tag pu-tag-sm pu-tag-blue">presentation</span>
                    <span class="pu-tag pu-tag-sm pu-tag-blue">client facing</span>
                    <span class="pu-tag pu-tag-sm pu-tag-blue">demo</span>
                </p>
            </footer>
        </article>
    </div>
    <div class="col-sm-4">
        <div class="pu-cards">
            <article class="pu-card">
                <header class="pu-card-header">
                    <aside class="pu-card-header-aside">
                        <img class="thumbnail" src="../img/image.jpg" alt="">
                    </aside>
                    <main class="pu-card-header-main">
                        <h4>
                            <a href="#_">Sales presentation to Aqua Corp</a>
                        </h4>
                        <p class="gray">
                            <small>
                                <i class="fa fa-clock-o"></i>
                                19 Oct 15, 03:53 PM</small>
                        </p>
                    </main>
                </header>
                <footer class="pu-card-footer">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lorem ipsum dolor sit amet, consectetur adipiscing elit Lorem ipsum dolor sit amet, consectetur adipiscing elit
                </footer>
            </article>
            <article class="pu-card">
                <header class="pu-card-header">
                    <aside class="pu-card-header-aside">
                        <img class="thumbnail" src="../img/image2.jpg" alt="">
                    </aside>
                    <main class="pu-card-header-main">
                        <h4>
                            <a href="#_">Sales presentation to Aqua Corp</a>
                        </h4>
                        <p class="gray">
                            <small>
                                <i class="fa fa-clock-o"></i>
                                19 Oct 15, 03:53 PM</small>
                        </p>
                    </main>
                </header>
                <footer class="pu-card-footer">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit Lorem ipsum dolor sit amet, consectetur adipiscing elit ...
                </footer>
            </article>
        </div>
    </div>
</div>
<h2>Learning Card</h2>
<div class="row">
    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <h4>
                    <a href="#_">Engaging your Colleagues</a>
                </h4>
                <p>
                    <span class="label label-info">Online learning
                    </span>
                </p>
            </header>
            <section class="pu-card-content">
                Delivered a sales presentation. Got great feedback about my delivery from Senior Management. #dayne gave feedback about my presentation skills. Commented I need to work on time management and that I should ...
            </section>
        </article>
    </div>
    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <h4>
                    <a href="#_">Engaging your Colleagues</a>
                </h4>
                <p>
                    <span class="label label-info">Online learning
                    </span>
                </p>
            </header>
            <section class="pu-card-content">
                Delivered a sales presentation. Got great feedback about my delivery from Senior Management. #dayne gave feedback about my presentation skills. Commented I need to work on time management and that I should ...
            </section>
            <footer class="pu-card-footer">
                <i class="pu-icon pu-icon-thumbs-up"></i>
                15
            </footer>
        </article>
    </div>
    <div class="col-sm-4">
        <article class="pu-card">
            <header class="pu-card-header">
                <div class="actions">
                    <div class="checkbox checkbox-circle checkbox-success">
                        <input type="checkbox">
                        <label></label>
                    </div>
                    <div class="more">
                        <a href="#" data-toggle="dropdown" class="dropdown-toggle">
                            <i class="fa fa-ellipsis-h fa-lg" aria-hitden="true"></i>
                        </a>
                        <ul class="dropdown-menu dropdown-menu-right">
                            <li>
                                <a href="#">Action 1</a>
                            </li>
                            <li>
                                <a href="#">Action 2</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <h4>
                    <a href="#_">Engaging your Colleagues</a>
                </h4>
                <p class="gray">
                    <span class="label label-info">Reading
                    </span>
                </p>
            </header>
            <figure class="pu-card-figure">
                <img class="featured" src="../img/image2.jpg" alt="">
            </figure>
            <section class="pu-card-content">
                Delivered a sales presentation. Got great feedback about my delivery from Senior Management. #dayne gave feedback about my presentation skills. Commented I need to work on time management and that I should ...
            </section>
            <footer class="pu-card-footer">
                <i class="pu-icon pu-icon-thumbs-up"></i>
                15
            </footer>
        </article>
    </div>
</div>

<?php endblock()?>
