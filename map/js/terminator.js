// Copyright (c) 2012 KASHIWAI, Isana
...
GST.Time = GST.Time || function(date){
...
GST.Terminator = function(option){
  ...
  var _setTerminator = function(time){
  ...
  //Generate Porygon Array
    ...
    var _nightPathFactory = function(lng_array,lat_array,start_lat){
      ...
      if (lng_array_length>0){
        if(polar_night_flag!="none"){
          polygon.push(new google.maps.LatLng(lat_limit,lng_array[0]));
        }
        polygon.push(new google.maps.LatLng(start_lat,lng_array[0]));
        for (var i = 0; i < lat_array_length; i++) {
          polygon.push(new google.maps.LatLng(lat_array[i],lng_array[i]));    
          polyline.push(new google.maps.LatLng(lat_array[i],lng_array[i]));    
        }
        polygon.push(new google.maps.LatLng(start_lat,lng_array[lng_array_length-1]));
       
        if(polar_night_flag!="none"){polygon.push(new google.maps.LatLng(lat_limit,lng_array[lng_array_length-1]));}
    ...
    for(var j=0;j<al;j++){
      var p = new _nightPathFactory(splitted_lng_array[j],splitted_lat_array[j],start_lat);
  ...
  var _removeTerminator= function(){
    if (shade){
      for(var i = 0, data_length = GST.Storage.NightPorigon.length; i<data_length;i++){
        GST.Storage.NightPorigon[i].setMap(null);
      }
    }
    if (boundary){
      for(var i = 0, data_length = GST.Storage.NightBoundaryPolyline.length; i<data_length;i++){
